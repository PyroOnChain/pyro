...
contract Deploy is Script {
    ...
    function run() external {
        ...
        // Deploy owned by the wallet doing the deploying, so the setMinSeed calls below
        // are authorised, then hand ownership over at the end. Deploying straight to a
        // multisig would make every setup call revert with OwnableUnauthorizedAccount.
        address deployer = msg.sender;

        vm.startBroadcast();
        ClubFactory factory = new ClubFactory(IPonsFactory(PONS_FACTORY), treasury, guardian, deployer);
        ...
        bool handover = owner != deployer;
        if (handover) factory.transferOwnership(owner);

        vm.stopBroadcast();

        console2.log("ClubFactory:", address(factory));
        console2.log("owner      :", handover ? owner : deployer);
        console2.log("guardian   :", guardian);
        console2.log("treasury   :", treasury);
        if (handover) {
            console2.log("");
            console2.log("ACTION NEEDED: ownership is pending. The new owner must call");
            console2.log("acceptOwnership() on the factory before it can change any setting.");
        }
        console2.log("");
